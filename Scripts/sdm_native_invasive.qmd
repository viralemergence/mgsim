...
native_points <- native_thin %>% map(~transmute(., class = "presence")) %>% 
  map2(replicate(78, slice_sample(native_abs, n = 100), simplify = F), bind_rows)
invasive_points <- invasive_thin %>% map(~transmute(., class = "presence")) %>% 
  map2(replicate(78, slice_sample(invasive_abs, n = 100), simplify = F), bind_rows)
...
This is all well and good, but it would probably help to filter down to uncorrelated variables. I do so here.

```{r filter uncorrelated}
plan(multisession)
high_cor <- files2 %>% map(rast) %>% map(filter_high_cor)
high_cor <- high_cor %>% 
  map(function(x) {
    case_when(
      str_detect(x, "AD") ~ "urban",
      str_detect(x, "_1$") ~ "bio1",
      str_detect(x, "_2$") ~ "bio2",
      str_detect(x, "_3$") ~ "bio3",
      str_detect(x, "_4$") ~ "bio4",
      str_detect(x, "_5$") ~ "bio5",
      str_detect(x, "_6$") ~ "bio6",
      str_detect(x, "_7$") ~ "bio7",
      str_detect(x, "_8$") ~ "bio8",
      str_detect(x, "_9$") ~ "bio9",
      str_detect(x, "_10") ~ "bio10",
      str_detect(x, "_11") ~ "bio11",
      str_detect(x, "_12") ~ "bio12",
      str_detect(x, "_13") ~ "bio13",
      str_detect(x, "_14") ~ "bio14",
      str_detect(x, "_15") ~ "bio15",
      str_detect(x, "_16") ~ "bio16",
      str_detect(x, "_17") ~ "bio17",
      str_detect(x, "_18") ~ "bio18",
      str_detect(x, "_19") ~ "bio19"
    )
  })
high_cor <- high_cor %>% flatten_chr() %>% unique()
high_cor
```

This brings us down to 10 predictors instead of 20.

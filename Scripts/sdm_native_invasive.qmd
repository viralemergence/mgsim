...
Here I have the specific goal of producing a stack of habitat suitability maps to use for simulations later. First I will test one year, 2000, to see how it looks.

```{r predict 2000}
predict2000 <- full_ensemble %>% 
  predict_raster(
    rast("~/Documents/Very_Large_Data/predictors/predictors_2000.grd") %>% 
      `names<-`(col_names), 
    type = "prob",
    fun = "weighted_mean",
    metric_thresh = c("tss_max", 0.7))
ggplot() + 
  geom_spatraster(data = predict2000, aes(fill = weighted_mean)) +
  scale_fill_paletteer_c(`"scico::bamako"`, direction = -1) +
  geom_sf(data = native_extract %>% filter(class == "presence",
                                           Year == "2000"), alpha = 0.3) +
  geom_sf(data = invasive_extract %>% filter(class == "presence",
                                             Year == "2000"), alpha = 0.3) +
  ggtitle("Probability of occurrence 2000")
```
...
suitability <- map(files2, rast) %>% 
  ...
  rast() %>% rescale0to1()
terra::writeRaster(suitability, here("Data/Input/habitat_suitability_v1.tif"))
```

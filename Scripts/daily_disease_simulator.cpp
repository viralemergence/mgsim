...
arma::mat daily_siri_summer(
  ...
  Rcpp::Rcout << "Number of populations: " << n_pops << std::endl;

  for (int p = 0; p < n_pops; p++) {
    Rcpp::Rcout << "Processing population: " << p << std::endl;
    Rcpp::Rcout << "Season length: " << season_length(p) << std::endl;
    if (season_length(p) == 0) {
      Rcpp::Rcout << "Skipping population due to non-positive season length." << std::endl;
      continue;
    }
    ...
    if (population_size == 0) {
      Rcpp::Rcout << "Skipping population due to zero population size." << std::endl;
      continue;
    }
    // Prep the parameters
    double birth_rate = fecundity / season_length(p);
    Rcpp::Rcout << "Birth rate: " << birth_rate << std::endl;
    arma::vec dd_mortality(n_stages);
    dd_mortality(0) = mortality_Sj_summer / season_length(p);
    dd_mortality(1) = mortality_Sa_summer / season_length(p);
    dd_mortality(2) = mortality_I1j_summer;
    dd_mortality(3) = mortality_I1a_summer;
    dd_mortality(4) = mortality_Rj_summer / season_length(p);
    dd_mortality(5) = mortality_Ra_summer / season_length(p);
    dd_mortality(6) = mortality_I2j_summer;
    dd_mortality(7) = mortality_I2a_summer;
    Rcpp::Rcout << "dd_mortality: " << dd_mortality.t() << std::endl;
    ...
    for (int t = 0; t < season_length(p); t++) {
      ...
      for (int i = 0; i < n_stages; i++) {
        dd_mortality(i) = std::min((1.0 + N / carrying_capacity(p)) * dd_mortality(i), 1.0);
      }

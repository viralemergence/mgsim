...
library(doParallel)
library(foreach)
i_am("mgsim/Scripts/round2d_validation.R")
...
results_dir <- system("cd /glade/work/pilowskyj/Round2d/; ls -f", intern = T) |>
  gtools::mixedsort() |>
  _[-c(1:2)]
...
dc <- results_dir |>
  map(\(path) {
    system(
      paste0("cd /glade/work/pilowskyj/Round2d/", path, "; ls -f"),
      intern = T
    )
  }) |>
  map(length) |>
...
infected_list <- results_dir[dc] |>
  map(\(f) paste0("/glade/work/pilowskyj/Round2d/", f)) |>
...
mg_presence_metric <- foreach(
  ...
  .combine = c,
  .packages = c("dplyr", "purrr", "qs2", "here")
) %dopar% {
...
write_csv(
  summary_metrics,
  here("mgsim/Data_minimal/Validation/round2d_validation_metrics.csv")
)
write_csv(
  trend_metric,
  here("mgsim/Data_minimal/Validation/round2d_trend_metrics.csv")
)

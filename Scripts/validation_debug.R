...
presabs_metric <- foreach(
  ...
  tryCatch({
    ...
    for (i in seq_along(plist_filled)) {
      ...
      # Print the year and number of paths being processed
      print(paste("Processing year:", df$Year[1], "with", length(df$path), "files at", Sys.time()))
      flush.console()

      combined_files <- Reduce(`+`, lapply(df$path, read_or_zero))
      sim_array[, , i] <- combined_files

      # Print after processing each year
      print(paste("Processed year", i, "at:", Sys.time()))
      flush.console()
    }

...
initial_abundance[39, 118] <- sim$house_finch$traits$initial_release
sim$add_traits(
  species = "house_finch",
...
sim$add_process(
  ...
  process_fun = function() {
    ...
    season_length <- self$sim$environment$current$breeding_season_length[occupied_indices]

    # Convert the units of anything seasonal to now be daily
    fecundity <- replicate(active_pops, list(self$traits$fecundity)) |>
    ...
    transmission <- replicate(active_pops, list(c(
      self$traits$beta_Sj_summer, self$traits$beta_Sa_summer, 0, 0,
      self$traits$beta_Rj_summer, self$traits$beta_Ra_summer, 0, 0
    )))
    recovery <- replicate(active_pops, list(c(
      0, 0, self$traits$recovery_I1j_summer, self$traits$recovery_I1a_summer, 0, 0,
      self$traits$recovery_I2j_summer, self$traits$recovery_I2a_summer
    )))

    population_new <- future_pmap(list(initial_pop = population_list[occupied_indices],
                              mortality = mortality,

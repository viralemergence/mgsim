...
library(here)
library(doParallel)
i_am("mgsim/Scripts/simulation_round2e.r")
data_dir <- here("mgsim/Data_minimal/Input")
results_dir <- "/glade/work/pilowskyj/Round2e" # production
...
incomplete_sims <- c(
  ...
  setdiff(1:10000, run_indices)
) |>
  sort()
write_csv(
  data.frame(sim_index = incomplete_sims),
  file.path("/glade/work/pilowskyj/incomplete_sims_round2e.csv")
)
results_dir <- "/glade/work/pilowskyj/Round2e"
...
sample_data <- read_csv(file.path(data_dir, "sample_data_round2e.csv")) |>
  mutate(

...
library(here)
library(doParallel)
i_am("mgsim/Scripts/posterior_predictive_simulations.R")
data_dir <- here("mgsim/Data_minimal/Input")
results_dir <- "/glade/work/pilowskyj/posterior_predictive" # production
set_trust_promises(TRUE)
random_seed <- 90
n_sims <- 1000 # production
# n_sims <- 10 # testing
region <- data_dir |> file.path("finch_region.qs") |> qread()

#### Create sample data frame ####
sample_data <- read_csv(file.path(
  data_dir,
  "posterior_predictive_priors.csv"
)) |>
  mutate(

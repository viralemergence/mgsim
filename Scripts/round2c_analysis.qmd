...
# Finch first arrival and Mycoplasma presence

I chose these two validation metrics, year of house finch arrival and Mycoplasma presence, using `abctools::selectsumm` to find the metrics that narrowed the posterior distribution the most.
...
abc35_processed <- process_sims(selected_samples_2, abc35$weights, here("Data/OUtput/Round2c"))
animate_sim(abc35_processed$total_population, region)
```

This animation looks much better. And now we will look at prevalence to get a sense of the outbreak of *Mycoplasma*:

```{r animate_recovered_adults}
animate_sim(abc35_processed$prevalence, region, years = 1994:2016, burn_in = 111)
```

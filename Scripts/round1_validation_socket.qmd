...
library(tidyr)
library(doParallel)
i_am("mgsim/Scripts/round1_validation_socket.qmd")
# Register parallel backend
num_cores <- 125
cl <- makeCluster(num_cores)
registerDoParallel(cl)
results_dir <- system("cd /glade/work/pilowskyj/Round1_matrix; find $(pwd) -maxdepth 1 -type d", intern = TRUE) |>
  _[-1] |> discard(\(string) str_detect(string, "txt")) |> gtools::mixedsort()
...
fill_missing_years <- function(df, start_year, end_year) {
  missing_path <- "/glade/u/home/pilowskyj/mgsim/Data_minimal/Validation/dummy_result.qs"
...
# Are birds in DC?
dc <- results_dir |> 
  map(\(path) system(paste0("cd ", path, "; ls -f"), 
                     intern = T)) |> 
  map(length) |> 
  map_lgl(\(x) x > 214)
# Graph
...
presabs_metric <- foreach(
  ...
  # Pre-allocate a 3D array for results rather than a list
  sim_array <- array(0, dim = c(106, 161, length(plist_filled)))
  
  # Process each grouped dataframe separately
  for (i in seq_along(plist_filled)) {
    df <- plist_filled[[i]]
    
    combined_files <- Reduce(`+`, lapply(df$path, qread))
...
bcr <- here("mgim/Data_minimal/Validation/bird_conservation_regions.qs") |> qread()

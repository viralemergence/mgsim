...
round2_metrics$hm_trend_1970[round2a_metrics$dc] <- round2_metrics$hm_trend[seq(1, 9900, 2)]
round2_metrics$hm_trend_1993[round2a_metrics$dc] <- round2_metrics$hm_trend[seq(2, 9900, 2)]
...
round2_priors <- read_csv(here("Data_minimal/Input/sample_data_round2a.csv"))
```
...
ensemble_sa <- ensemble_mean(selected_samples, abc50$weights, "Sa", here("Data/Output/Round2"))
animate_sim(ensemble_sa, region)
```

Now let's look at adults infected for the first time to get a sense of the outbreak:

```{r animate_infected1_adults}
ensemble_I1a <- ensemble_mean(selected_samples, abc49$weights, "I1a", here("Data/Output/Round1_matrix"))
animate_sim(ensemble_I1a, region)
```

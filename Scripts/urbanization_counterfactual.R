...
library(here)
library(doParallel)
i_am("mgsim/Scripts/urbanization_counterfactual.R")
...
abundance_gen <- Generator$new(
  ...
  hs_file = "habitat_suitability_counterfactual_urban1940",
...
landscape <- c(
  ...
  file.path(data_dir, "habitat_suitability_counterfactual_urban1940.tif")
) |>
  map(rast) |>
  map(\(x) x[[1:77]]) |>
  map_at(1, round) |>

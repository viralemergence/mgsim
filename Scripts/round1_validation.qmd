...
simulation_id <- results_dir[dc] |> str_extract("[0-9]+") |> as.integer()
...
process_single_number <- function(df, number) {
  if (number %in% df$number) {
    ...
    data.frame(healthy = rep(0), sick = rep(0), index = rep(number), region_cell = mg_trends$region_cell)
  }
}
...
mg_arrival <- here("mgsim/Data_minimal/Validation/mycoplasma_first_arrival.csv") |> 
  ...
  filter(first_arrival_low < 2017) |>
  mutate(first_arrival_high = if_else(first_arrival_high > 2016, 2016, first_arrival_high)) |>

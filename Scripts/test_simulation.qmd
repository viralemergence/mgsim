...
source(here::here("Scripts/animate_sim.R"))
animate_sim(qread(file.path(results_dir, "sample_1_results.qs"))$abundance,
            region, burn_in = 5, years = 1940:1993)
```

# Finch + MG simulation

Now that I have successfully run a round of 10,000 simulations of just the house finch, I want to try a single test simulation of the system from 1994 - 2016 with the bacteria in the mix. I will demonstrate this using simulation #4615, which is not really an accurate representation of the house finch at this time, but it's close enough for a test run.

## Input Data

### Model settings

These have to change from the first round.
...
### The parts that don't need to change

These input data are about the study region in space, which does not change in 1994.
...
This has to change because the years are different now.
...
This object stores fixed parameters for the model (ones that are not sampled via Latin hypercube sampling.) A bunch of it needs to be changed now that there's an outbreak.
...
This generator can take input abundances and shift them around according to dispersal rules. This part is the same as in the first round.
...
This object is a generator. This means that it generates complex outputs (in this case, a matrix of carrying capacity throughout the simulation and a matrix of initial abundance) based on simple inputs (in this case, maximum population density and number of finches initially infected in Washington, DC.)
...
I am only going to run one test simulation for now. However, I still want to generate a data set sampling all the variable parameters. Again, the sample data is changed from round 1 because we have an outbreak now.
...
sample_data <- lhs_generator$generate_samples(number = nsims, 
    ...
    left_join(round1_sample_data, by = c("abundance_file" = "sample"))
```

## Simulation

This simulation is based on run #4615, which does have presence in DC at the right timestep.

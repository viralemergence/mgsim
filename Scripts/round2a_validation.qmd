...
library(doParallel)
library(foreach)
i_am("mgsim/Scripts/round2_validation.qmd")
...
results_dir <- system("cd /glade/work/pilowskyj/Round2/; ls -f", intern = T) |>
  gtools::mixedsort() |>
  _[-c(1:2)]
round2_priors <- read_csv(here(
  "mgsim/Data_minimal/Input/sample_data_round2.csv"
))
...
dc <- results_dir |>
  map(\(path) {
    system(
      paste0("cd /glade/work/pilowskyj/Round2/", path, "; ls -f"),
      intern = T
    )
  }) |>
  map(length) |>
...
prevalence_trend_metric <- function(df) {
  prevalence_metrics <- df |>
    ...
    mutate(
      model = map(data, \(df) {
        glm(cbind(sick, healthy) ~ Year, family = binomial, data = df)
      })
    ) |>
...
prevalence_trend <- sick_healthy |>
  group_split(sim) |>
  future_map_dbl(prevalence_trend_metric)
...
infected_list <- results_dir[dc] |>
  map(\(f) paste0("/glade/work/pilowskyj/Round2/", f)) |>
...
write_csv(
  summary_metrics,
  here("mgsim/Data_minimal/Validation/round2_validation_metrics.csv")
)
round2_priors |>
  mutate(dc = dc) |>

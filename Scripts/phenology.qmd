...
There are a lot of predictions to make, because I have 6986 region cells over 78 years.
...
season_predict <- predict_extract %>% bind_cols(season_predict) %>% 
...
In accordance with the R squared, overall the bioclim model predictions look better than the Latitude + Year predictions. But as a final check, I want to try a random forest model with all of these predictors to see if that is any better.
...
exclude <- tree_rec$steps[[1]]$exclude
...
rf_fit <- fit(rf_workflow, data = training(full_split))

last_fit(rf_workflow, full_split, metrics = metric_set(rsq, mpe)) %>% 
  collect_metrics()

full_predict <- predict(rf_fit, new_data = predict_extract %>% 

...
results_dir <- "/glade/work/pilowskyj/Round1.5"
set_trust_promises(TRUE)
random_seed <- 90
n_sims <- 10000
region <- data_dir |> file.path("finch_region.qs") |> qread()
incomplete_sims <- read.delim("/glade/work/pilowskyj/incomplete_simulations.txt", sep = "\n") |> as.vector()
...
numCores <- 72
cl <- startMPIcluster(count=numCores, verbose=TRUE)
registerDoMPI(cl)

...
Here I collect trends in prevalence from throughout the US. There are more data points in the East, but there are some throughout.

```{r prevalence trends}
hfds1 <- vroom(
  "~/Documents/Very_Large_Data/House Finch Disease Survey/HFDSPapper_HOFIEyeDisease_Pilowsky.csv"
)
hfds2 <- vroom(
  "~/Documents/Very_Large_Data/House Finch Disease Survey/PFW_HOFIEyeDisease_Pilowsky.csv"
)
hfds <- hfds2 |>
  filter(symptom_code %in% c("red_crusty_eyes")) |>
  group_by(ID, Year, Month) %>%
  summarise(
    NDaysObserved = sum(NDaysObserved),
    Total_Birds = max(NBirdsTot),
    Sick_Birds = max(NBirdsSick)
  ) |>
  left_join(
    hfds2 |>
      select(
  ...
  mutate(problem = Sick_Birds > Total_Birds) |>
  filter(!problem) |> 
  select(-problem) |> 
  ungroup()
...
trends <- hfds |> 
  ...
  mutate(sumyears = length(unique(Year)), sumbirds = sum(Total_Birds)) %>% 
  filter(sumbirds > 0, sumyears == 15) %>% arrange(region_cell) %>%
  mutate(Healthy_Birds = Total_Birds - Sick_Birds) %>%
  nest() %>%
  mutate(model = map(data, ~ glm(
    cbind(Sick_Birds, Healthy_Birds) ~ Year, family = binomial, data = .
  ))) %>%
...
sightings <- hfds |> mutate(region_cell = rasterized) |> 
  filter(Sick_Birds > 0, region_cell %in% region$region_indices) |> 
  group_by(region_cell) |> 
...
arrivals <- sightings |> 
  map(possibly(\(s) weib_percentile_ci(s$Year, 250, 0.10, 100, parallelize = "snow", ncpus = 80)))
arrivals <- sightings |>
  map(possibly(\(s) quantile_ci(s$Year, 0.05)))
mg_arrival <- sightings |>
  map2(arrivals, \(x, y) mutate(x, first_arrival_low = y[[2]],
                                first_arrival_high = y[[3]],
                                first_arrival = y[[1]])) |>
  bind_rows() |>
  filter(!is.na(first_arrival), first_arrival_high > 1996)
...
pres_hfds <- hfds |> mutate(region_cell = rasterized) |> 
  filter(Sick_Birds > 0, region_cell %in% region$region_indices, Year < 2017) |> 
  filter(Sick_Birds > 0, region_cell %in% region$region_indices) |> 
  group_by(region_cell, Year) |> 
  summarize(total_sick = sum(Sick_Birds)) |> 
  summarize(total_sick = sum(Sick_Birds)) |> 
  ...
  # Adding Mycoplasma presence from Montana to add more western points (Dhondt et al. 2006)
  add_row(region_cell = matrix(c(-114, 46.867), nrow = 1) |>  sf_project(from = "EPSG:4326", to = "+proj=aea +lat_0=34.5 +lon_0=-94.5 +lat_1=21.5
+ +lat_2=47.5 +x_0=0 +y_0=0 +datum=WGS84 +units=m
+ +no_defs") |> cellFromXY(object = region_raster), 
         min_year = 2002, max_year = 2004,
         min_index = which(1994:2016 == min_year),
         max_index = which(1994:2016 == max_year))

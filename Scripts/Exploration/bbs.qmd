...
hofi <- here("Data/Validation/FeederWatch/States") %>% list.files(full.names = T) %>% 
...
presabs <- here("Data/Validation/FeederWatch/States") %>% list.files(full.names = T) %>% 
  map(vroom, col_types = "iiiciiciiiii") %>% 
  bind_rows() %>% 
  filter(Year<2020) %>% 
  left_join(routes) %>% 
...
NA_map <- ggplot() +
  ...
  coord_sf(xlim = c(-140, -50), ylim = c(25, 55))
gc()
```

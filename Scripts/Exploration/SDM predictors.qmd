...
#   if (!file.exists(paste0('~/Documents/Very Large Data/', "BioClim_", x, ".nc"))) {
#     BioClim(
#       Y_start = x,
#       Y_end = x,
#       T_res = 'day',
...
urban_rasters <- "~/Documents/Very_Large_Data/baseline/zip/" %>%
...
Now I will write a script to go through all the bioclim stacks and add a layer for each year giving the urbanization index.

```{r script}
files <- "~/Documents/Very Large Data/ERA5" %>% 
  list.files(full.names = T) %>%
  .[1:78]
plan(multisession)
future_walk(1:length(files), function(i) {
  if (!file.exists(paste0(
    "~/Documents/Very Large Data/predictors/predictors_",
    ts[i],
    ".grd"
  ))) {
    ...
    urban_proj <- project(urban_raster, bioclim_proj, "sum") / 2151
    writeRaster(
...
library(vroom)
hofi_sample <- vroom("~/Documents/Very Large Data/hofi_gbif_qc.tsv") %>% 
...
82.5% overlap in the niche space of the native and invasive, which means I can probably safely pool the data across the range for the SDM.
...
vroom_write(partition, "~/Documents/Very Large Data/gbif_hofi_predictors.csv")
...
bind_rows(background_inv, background_nat) %>% 
  vroom_write("~/Documents/Very Large Data/hofi_background_points.csv")
```

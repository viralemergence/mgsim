...
library(doParallel)
library(foreach)
i_am("mgsim/Scripts/posterior_predictive_validation.r")
...
results_dir <- system("cd /glade/work/pilowskyj/posterior_predictive/; ls -f", intern = T) |>
  gtools::mixedsort() |>
  _[-c(1:2, 10003)]
...
dc <- results_dir |>
  map(\(path) {
    system(
      paste0("cd /glade/work/pilowskyj/posterior_predictive/", path, "; ls -f"),
      intern = T
    )
  }) |>
  map(length) |>
...
infected_list <- results_dir[dc] |>
  map(\(f) paste0("/glade/work/pilowskyj/posterior_predictive/", f)) |>
...
summary_metrics <- data.frame(index = 1:1001, dc = dc)
...
write_csv(
  summary_metrics,
  here("mgsim/Data_minimal/Validation/posterior_predictive_validation_metrics.csv")
)

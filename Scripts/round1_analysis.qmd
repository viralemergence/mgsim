...
## 3.e. Point prevalence, finch arrival, and both finch trends

I come back with the benefit of hindsight from many rounds of modeling and have come to the conclusion that spatiotemporal point prevalence, finch first arrival, finch trend from 1970, and finch trend from 1993 are the most difficult aspects for the simulations to get right, and therefore are the most important to optimize on, so I'm going to try this combo of validation metrics out.

```{r 4parameters}
abc49 <- abc(param = round1_priors[round1_metrics$dc,], 
             sumstat = round1_metrics_dc[, c(3, 5:7)],
              target = rep(0, 4), 
             tol = 0.01, 
             method = "neuralnet")
head(abc49$ss, 10)
...
ensemble_model <- ensemble_time_series(selected_samples, here("Data/Output/Round1_matrix")) |> 
...
The population size trends look a little bit questionable, though at least the in-filling of finch populations from 1940 looks pretty good. Let's check out prevalence.
...
The spread of the disease looks way too quick in this animation, it's no good.

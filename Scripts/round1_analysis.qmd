...
round1_metrics <- read_csv(here("Data_minimal/Validation/round1_validation_metrics.csv")) |>
...
abc49 <- abc(param = round1_priors[round1_metrics$dc,], 
             ...
             tol = 0.01, 
             method = "neuralnet")
qsave(abc49, here("Data/Validation/abc_round1.qs"))
head(abc49$ss, 10)
...
write_sftp_transfer("/glade/work/pilowskyj/Round1_matrix", 
                      "/Users/julypilowsky/Documents/mgsim/Data/Output/Round1_matrix", 
                      selected_samples, 
...
ensemble_sj <- ensemble_mean(selected_samples, abc49$weights, "Sj", here("Data/Output/Round1_matrix"))
ensemble_rj <- ensemble_mean(selected_samples, abc49$weights, "Rj", here("Data/Output/Round1_matrix"))
ensemble_I1a <- ensemble_mean(selected_samples, abc49$weights, "I1a", here("Data/Output/Round1_matrix"))
ensemble_I2a <- ensemble_mean(selected_samples, abc49$weights, "I2a", here("Data/Output/Round1_matrix"))
ensemble_I1j <- ensemble_mean(selected_samples, abc49$weights, "I1j", here("Data/Output/Round1_matrix"))
ensemble_I2j <- ensemble_mean(selected_samples, abc49$weights, "I2j", here("Data/Output/Round1_matrix"))
...
abc49 <- abc(param = round1_priors[round1_metrics$dc,], 
             ...
             method = "neuralnet")
head(abc49$ss, 10)
qs_save(abc49, here("Data/Validation/abc_round1_2variable.qs2"))
selected_samples <- round1_priors %>% 
  rowid_to_column("sample") |> 
  semi_join(abc49$unadj.values, copy = TRUE) %>% 
  pull(sample)
write_sftp_transfer("/glade/work/pilowskyj/Round1_matrix", 
                "/Users/julypilowsky/Documents/mgsim/Data/Output/Round1_matrix", 
                      selected_samples, 
                      filename=here("Scripts/Globus/abc2.txt"))
...
sim_weights <- data.frame(sim = selected_samples, weight = abc49$weights)
ensemble_model <- ensemble_time_series(
  selected_samples,
  here("Data/Output/Round1_matrix")
) |>
...
p +
  geom_line(
    ...
    aes(x = Time, y = Abundance, group = NA),
    color = "red",
    linewidth = 1,
    alpha = 0.8,
    inherit.aes = FALSE
...
abc49 <- abc.stu(
  param = round1_priors[round1_metrics$dc, ],
...
qs_save(abc49, here("Data/Validation/abc_round1_5params.qs2"))
head(abc49$ss, 10)
selected_samples <- round1_priors %>%
  rowid_to_column("sample") |>
  semi_join(abc49$unadj.values, copy = TRUE) %>%
  pull(sample)
write_sftp_transfer(
  "/glade/work/pilowskyj/Round1_matrix",
  "/Users/julypilowsky/Documents/mgsim/Data/Output/Round1_matrix",
  selected_samples,
  here("Scripts/Globus/abc3.txt")
)
everything_processed <- process_sims(
  selected_samples,
  abc49$weights,
  here("Data/Output/Round1_matrix")
)
total_population_size <- everything_processed[[1]]
animate_sim(total_population_size, region, years = 1940:2016)
```

This run looks similar in total population size trends to others but with lower abundance and higher seasonality. But let's look at prevalence to get a sense of the outbreak:
...
The spread of the disease also seems to be much too fast in this one.

```{r everything posteriors}
plot_abc_posteriors(abc49, round1_priors)
```

## 3.e. Point prevalence, finch arrival, and Mycoplasma prevalence

I come back with the benefit of hindsight from many rounds of modeling and have come to the conclusion that spatiotemporal point prevalence would help keep a lid on prevalence going so high in the Mycoplasma presence/finch arrival simulations (3c, above.)

```{r 4parameters}
abc49 <- abc.stu(
  param = round1_priors[round1_metrics$dc, ],
  sumstat = round1_metrics_dc[, c(1, 3, 5)],
  target = rep(0, 3),
  tol = 0.01,
  scaling = "sd",
  method = "neuralnet"
)
qs_save(abc49, here("Data/Validation/abc_round1_3params.qs2"))
head(abc49$ss, 10)
selected_samples <- round1_priors %>%
  rowid_to_column("sample") |>
  semi_join(abc49$unadj.values, copy = TRUE) %>%
...
write_sftp_transfer(
  ...
  "/Users/julypilowsky/Documents/mgsim/Data/Output/Round1_matrix",
  selected_samples,
  here("Scripts/Globus/abc4.txt")
)
param4_processed <- process_sims(
  selected_samples,
  abc49$weights,
  here("Data/Output/Round1_matrix")
)
total_population_size <- param4_processed[[1]]
animate_sim(total_population_size, region, years = 1940:2016)
...
The population size trends look questionable, as the population size actually increases after MG is introduced, though at least the in-filling of finch populations from 1940 looks pretty good. Let's check out prevalence.

```{r 4parameters_prevalence}
prevalence <- param4_processed[[2]]
animate_sim(prevalence, region, years = 1994:2016, burn_in = 111)
```

The spread of the disease looks good, though the prevalence might be a little TOO low. There's room for improvement, though.
